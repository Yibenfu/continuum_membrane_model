...
#include "functions_for_triangular_mesh.cpp"
...
void Energy_and_Force(mat vertex, mat vertexold, Mat<int> nearby_vertices, Mat<int> vertexi_face, Mat<int> face, Mat<int> A, Param& param, rowvec spontcurv, Mat<int> insertionpatch, Row<int> isinsertionpatch, Row<double>& E, mat& F, rowvec& deformnumbers, rowvec gqcoeff, cube shape_functions) {
    ...
    //cout << "deformsnumber = "<<deformnumbers(0)<<", "<<deformnumbers(1)<<", "<<deformnumbers(2)<<endl;
    ///////////////////////////////////////////////////////////////////////////
...
void check_nodal_force(mat vertex, mat vertexref, Mat<int> vertexi_nearby, Mat<int> vertexi_face, Mat<int> face, Mat<int> face_ring_vertex, Param param, rowvec spontcurv, Mat<int> insertionpatch, Row<int> Isinsertionpatch, rowvec gqcoeff, cube shape_functions){
    cout<<"check if the nodal force is correct: "<<endl;
    int numvertex = vertex.n_rows;
    rowvec energy(6); energy.fill(0);
    mat force(numvertex,3); force.fill(0);
    rowvec deformnumbers(3);
    Energy_and_Force(vertex, vertex, vertexi_nearby, vertexi_face, face, face_ring_vertex, param, spontcurv, insertionpatch, Isinsertionpatch, energy, force, deformnumbers, gqcoeff, shape_functions);  
    double E = energy(5);
    mat forceha(numvertex,3); 
    ...
    for (int i =0; i<numvertex; i++) {
        ...
        Energy_and_Force(vertextry, vertexref, vertexi_nearby, vertexi_face, face, face_ring_vertex, param, spontcurv, insertionpatch, Isinsertionpatch, energy, forceha, deformnumbers, gqcoeff, shape_functions);
...
/*
void check_nodal_force(mat vertexin, mat vertexout, Mat<int> vertexi_nearby, Mat<int> vertexi_face, Mat<int> face, Mat<int> face_ring_vertex, Param param, rowvec spontcurv, Mat<int> insertionpatch){
    cout<<"check if the nodal force is correct: "<<endl;
    int numvertex = vertexin.n_rows;
    rowvec energy(6); energy.fill(0);
    mat forcein(numvertex,3); forcein.fill(0);
    mat forceout(numvertex,3); forceout.fill(0);
    mat force(2.0*numvertex,3); force.fill(0);
    rowvec deformnumbers(3);
    Energy_and_Force_global(vertexin, vertexin, vertexout, vertexout, vertexi_nearby, vertexi_face, face, face_ring_vertex, param, spontcurv, insertionpatch, energy, forcein, forceout, force, deformnumbers);
    vec forcescale = force_scale(forcein);   
    double E = energy(5);
    mat forceinha(numvertex,3);
    mat forceoutha(numvertex,3);
    ...
    // inner layer 
  {
    ...
    for (int i =0; i<numvertex; i++) {
        ...
        double x=vertexin(i,0);
        double y=vertexin(i,1);
        double z=vertexin(i,2);
        double r = sqrt(x*x+y*y+z*z);
        double theta = acos( z/r );
        double phi = acos( x/(r*sin(theta)) ); if (theta < 1e-15 || abs(theta-M_PI)<1e-15) phi=0;
        if (y<0.0) phi = 2*M_PI - phi;
        double rnew = r+dr;
        vertextry(i,0) = rnew*sin(theta)*cos(phi);
        vertextry(i,1) = rnew*sin(theta)*sin(phi);
        vertextry(i,2) = rnew*cos(theta);
        Energy_and_Force_global(vertextry, vertexin, vertexout, vertexout, vertexi_nearby, vertexi_face, face, face_ring_vertex, param, spontcurv, insertionpatch, energy, forceinha, forceoutha, force, deformnumbers);
        double ff = - (energy(5)-E)/dr;
        forcereal(i,0)=ff *x/r;
        forcereal(i,1)=ff *y/r;
        forcereal(i,2)=ff *z/r;
        double frea = norm(forcereal.row(i),2.0);
    ...
    // outer layer 
  {
    vec forcescale = force_scale(forceout);
    mat forcereal(numvertex,3); forcereal.fill(0);
    double dr=1e-5;
    for (int i =0; i<numvertex; i++) {
        mat vertextry = vertexout;
        double x=vertexout(i,0);
        double y=vertexout(i,1);
        double z=vertexout(i,2);
        double r = sqrt(x*x+y*y+z*z);
        double theta = acos( z/r );
        double phi = acos( x/(r*sin(theta)) ); if (theta < 1e-15 || abs(theta-M_PI)<1e-15) phi=0;
        if (y<0.0) phi = 2*M_PI - phi;
        ...
        vertextry(i,1) = rnew*sin(theta)*sin(phi);
        vertextry(i,2) = rnew*cos(theta);
        Energy_and_Force_global(vertexin, vertexin, vertextry, vertexout, vertexi_nearby, vertexi_face, face, face_ring_vertex, param, spontcurv, insertionpatch, energy, forceinha, forceoutha, force, deformnumbers);
        double ff = - (energy(5)-E)/dr;
        forcereal(i,0)=ff *x/r;
        forcereal(i,1)=ff *y/r;
        forcereal(i,2)=ff *z/r;
