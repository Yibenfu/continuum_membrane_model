...
// basic parameters
double r   = 40.0;                     // sphere radius, nm
...
int main() {
    ...
    mat aa(2,3); aa.fill(0); //aa(0,1)=1;aa(0,2)=2;
      #pragma omp parallel for reduction(+:aa) 
    for (int i =0;i< nn; i++){
        //if (aa.n_cols<3){
        //    aa.set_size(1,3); aa.fill(0.0);aa(0,1)=1;aa(0,2)=2;
        //}
        if (omp_get_thread_num()==2 && i==0){
            //cout<<"thread 1: "<<setprecision(16)<<aa(0,0)<<", "<<aa(0,1)<<", "<<aa(0,2)<<endl;
        }
        for (int j=0;j<aa.n_rows;j++){
            rowvec temp(3); temp.fill(i/5.0);
            aa.row(j)=aa.row(j)+temp;
        }
    }
    cout<<setprecision(16)<<aa(0,0)<<", "<<aa(0,1)<<", "<<aa(0,2)<<aa(1,0)<<", "<<aa(1,1)<<", "<<aa(1,2)<<endl;
    cin.get();
    */
    ...
    Mat<int> insertionpatch; 
                 //insertionpatch << 806 << 162 << 807 << 402 << endr; // R=10 nm, no -1
                 //insertionpatch << 3029 << 2258 << 3017 << 579 << endr; // R=20 nm, no -1
                 insertionpatch << 19060 << 4646 << 19059 << 13525 << endr; // R=40nm, no -1
                 //insertionpatch << 58349 << 12623 << 58348 << 48131 << endr; // r=80nm; need -1 into c++ number
                 //insertionpatch << 100927 << 200384 << 39488 << 200382 << endr; // r=160, l=2; need -1 into c++ number
        //insertionpatch <<807<<163<<808<<403<<3700<<3699<<3701<<1768<<1767<<1769<<3703<<3702<<3704<<2488<<2487<<2489<<endr; //R=10nm, need -1
        //insertionpatch<<3029<<2258<<3017<<579<<14206<<14205<<14207<<11893<<11892<<11894<<14170<<14169<<14171<<6856<<6855<<6857<<endr; //R=20nm, need -1
        //insertionpatch<<19061<<4647<<19060<<13526<<77662<<77661<<77663<<34420<<34419<<34421<<77659<<77658<<77660<<61057<<61056<<61058<<endr;//R=40nm,need -1
    ...
    //rowvec elementS_target = setup_elementS_target(S0in, vertexin, face, insertionpatch); // element area of limit surface, not triangles
    rowvec spontcurv = determine_spontaneous_curvature(param, face, vertex, insertionpatch);
    printout_spontaneouscurvature(spontcurv);  
    
    //check_nodal_force(vertex, vertexref, vertexi_nearby, vertexi_face, face, face_ring_vertex, param, spontcurv, insertionpatch, Isinsertionpatch, gqcoeff,shape_functions);
    ...
    while ( isCriteriaSatisfied == false && i < N-1){
       ...
       } 
       {
           ...
           if (Ere_vs_Etot < 1.0e-9){
               //updateReference = false;
               //cout<<"Stop to update the reference structure! "<<endl;
           }
