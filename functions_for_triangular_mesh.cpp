...
/*
bool sharemember(rowvec vector1, rowvec vector2){
    bool out = false;
    for (int i = 0; i < vector1.n_cols; i++){
        int member1 = vector1(i);
        for (int j = 0; j < vector2.n_cols; j++){
            int member2 = vector2(j);
            if ( member1 == member2 )
                out = true;
        }
    }
...
rowvec spontaneous_curvature(Param param, Mat<int> face, mat vertex, rowvec insertionpatch){ 
    double C0 = param.C0;
    double c0 = param.c0;
    double R  = param.R;
    double sigma = param.sigma; // 2*sigma is the radial region of non-zero spontaneous curvature
    Mat<int> facetemp = face;
    rowvec spontcurv(face.n_rows); spontcurv.fill(c0);
    
    for (int i = 0; i < insertionpatch.n_cols; i++){
        int facenumber = insertionpatch(i);
        spontcurv(facenumber) = C0;
        facetemp(facenumber) = -1;
    }
    if ( sigma < 1e-9 ) 
        return spontcurv;
    
    // how many rings of triangles around the insertion
    double criterion = 1.0e-9;
    double x = sqrt( - 2.0*sigma*sigma* log(criterion/abs(C0)) );
    int ringnumber = round(x);
    // find ervery ring faces
    rowvec ringface_last = insertionpatch;
    for (int i = 0; i < ringnumber; i++){
        // find the ring face number
        int jishu = 0;
        rowvec ringface(face.n_rows); ringface.fill(-1);
        for (int j = 0; j < face.n_rows; j++){
            if (facetemp(j) == -1)
                continue;
            
            bool isRingElement = false;
            for (int k = 0; k < ringface_last.n_cols; k++){
                int facenumber = ringface_last(k);
                rowvec vec1(3); vec1(0) = face(j,0); vec1(1) = face(j,1); vec1(2) = face(j,2);
                rowvec vec2(3); vec2(0) = face(facenumber,0); vec2(1) = face(facenumber,1); vec2(2) = face(facenumber,2);
                isRingElement = sharemember(vec1,vec2);  
                if (isRingElement == true)
                    break;
            }
            if (isRingElement == true){
                ringface(jishu) = j;
        ...
        // calculate the spontaneous curvature value
        rowvec distvec(jishu);
        for (int j = 0; j < jishu; j++){
            int facenumber = ringface(j);
            rowvec center0 = 1.0/3.0 * ( vertex.row(face(facenumber,0)) + vertex.row(face(facenumber,1)) + vertex.row(face(facenumber,2)) );
            double dismin = 2.0*R;
            for (int k = 0; k < insertionpatch.n_cols; k++){
                int faceno = insertionpatch(k);
                rowvec center1 = 1.0/3.0 * ( vertex.row(face(faceno,0)) + vertex.row(face(faceno,1)) + vertex.row(face(faceno,2)) );
                rowvec disvec = center0 - center1;
                double dis = norm(disvec,2.0);
                double distance = 2.0*R * asin(dis/2.0/R);
                if (distance < dismin){
                    dismin = distance;
        ...
        double distmean = mean(distvec);
        for (int j = 0; j < jishu; j++){
            int facenumber = ringface(j);
            spontcurv(facenumber) =  - abs(C0) * exp( - pow(distmean/sigma,2.0)/2.0 );
            facetemp(facenumber) = -1; // mark these ring triangles
        }
        /////////////////////////////
        // update the ring faces of last ringsection
        ringface_last.set_size(jishu);
        for (int j = 0; j < jishu; j++){
            int facenumber = ringface(j);
            ringface_last(j) = facenumber;
        }
    }
...
rowvec determine_spontaneous_curvature(Param param, Mat<int> face, mat vertex, Mat<int> insertionpatch){
    ...
    // calculate the spontaneous curvature for each triangle as generated by each insertion zone.
    mat spontcurvs(insertionpatch.n_rows,face.n_rows);
    for (int i = 0; i < insertionpatch.n_rows; i++){
        rowvec insertion(insertionpatch.n_cols);
        for (int j = 0; j < insertionpatch.n_cols; j++){
            insertion(j) = insertionpatch(i,j);
        }
        spontcurvs.row(i) = spontaneous_curvature(param, face, vertex, insertion);
    }
    
    // calculate the efficient value of the spontaneous curvature 
    if ( sigma < 1e-15 ){
        for (int i = 0; i < face.n_rows; i++){
            spontcurv(i) = min(spontcurvs.col(i));
        }
        return spontcurv;
    } 
    ...
    for (int i = 0; i < face.n_rows; i++ ){
        if (isAdditiveScheme == true){
            ...
            double H = 1.0/R;
            for (int j = 0; j < insertionpatch.n_rows; j++){
                shu = shu + pow(2.0*H-(-spontcurvs(j,i)),2.0) - pow(2.0*H,2.0);
            }
            shu = shu + pow(2.0*H,2.0);
            if (shu < 0.0){ 
                cout<<"Note: when decide the enhanced spontaneous curvature, nonlinear effect happens. Then no additive scheme is utilized! "<<endl;
                ...
                //{
                double temp1 = -(2.0*H + sqrt(shu));
                double temp2 = -(2.0*H - sqrt(shu));
                if (temp1 < 0.0 && temp2 > 0.0){
                    spontcurv(i) = temp1;
                }else if (temp1 > 0.0 && temp2 < 0.0){
                    spontcurv(i) = temp2;
                }else{
                    rowvec temp12(2); temp12 << temp1 << temp2 << endr;
                    double themin = min(spontcurvs.col(i));
                    if (abs(temp1-themin)<abs(temp2-themin)){
                        spontcurv(i) = temp1;
                    }else{
                        spontcurv(i) = temp2;
                    }
            ...
            spontcurv(i) = min(spontcurvs.col(i)); // here the spont_curvature is negative sign
        }
    ...
    ofstream outfile("sponts.csv");
    for (int i = 0; i < face.n_rows; i++) {
        outfile << spontcurvs(0,i)+0.0 << ',' << spontcurvs(1,i)+0.0 << ',' << spontcurv(i)+0.0 << '\n';
    }
    outfile.close();
